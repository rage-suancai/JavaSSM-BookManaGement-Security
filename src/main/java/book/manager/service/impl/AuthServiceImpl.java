...
public class AuthServiceImpl implements AuthService {
    ...
    public void register(String name, String sex, String grade, String password) {
        ...
        if (mapper.addRegisterUser(user) <= 0) throw new RuntimeException(" 用户基本信息添加失败! ");
        if (mapper.addStudentInfo(user.getId(), name, grade, sex) <= 0) throw new RuntimeException(" 学生详细信息插入失败! ");
    }
